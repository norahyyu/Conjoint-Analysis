...
delete NA for profile 3,6,10,16
```{r}
...
```{r}
##Set up scenarios.  Here in this case we assume that companies just have one time to take action, no second,third,.... chance to react: 
scens = list()
scens[[1]]=c(7,3,13,15)      
scens[[2]]=c(7,3,15)   
scens[[3]]=c(7,3,13)        
...
### they betray us first, they only have one product to choose to drop price, we react with differnt low-price products combination, so we list all possibilitis###
scens[[5]]=c(8,3,13,15)      
scens[[6]]=c(8,3,15)   
scens[[7]]=c(8,3,13)        
scens[[8]]=c(8,13,15) 
...
profitCompetSet <- c()
for (j in 1:length(scens)){
  profitCompet <- share[[j]][[1]]*4000*profitMargin[scens[[j]][[1]]] - 20000
